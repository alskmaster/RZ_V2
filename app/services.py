...
# --- O REPORTGENERATOR ORQUESTRADOR E FERRAMENTAS COMPARTILHADAS ---
class ReportGenerator:
    ...
    def generate(self, client, ref_month_str, system_config, author, report_layout_json):
        ...
        final_html_parts = []
        if any(mod['type'] in availability_module_types for mod in report_layout):
        ...
        sla_module_config = next((mod for mod in report_layout if mod.get('type') == 'sla'), None)
        ...
        for module_config in report_layout:
            ...
            if collector_class:
                try:
                    ...
                    current_app.logger.error(f"Erro ao executar o plugin '{module_type}': {e}")
                    traceback.print_exc()
    ...
    def _collect_availability_data(self, all_hosts, period, sla_goal, trends_only=False):
        ...
        if ping_events is None: return None, "Falha na coleta de eventos de PING."
        ...
        if all_group_events is None: return None, "Falha na coleta de eventos gerais do grupo."
        ...
        sla_trend = 'stable'
        if avg_sla > prev_avg_sla: sla_trend = 'up'
        elif avg_sla < prev_avg_sla: sla_trend = 'down'

        incidents_trend = 'stable'
        if len(all_problems) < prev_all_problems_count: incidents_trend = 'up'
        elif len(all_problems) > prev_all_problems_count: incidents_trend = 'down'

        kpis_data = [
            {'label': f"Média de SLA ({len(hosts_for_sla)} Hosts)", 'value': f"{avg_sla:.2f}".replace('.', ',') + '%', 'sublabel': f"Meta: {f'{sla_goal:.2f}'.replace('.', ',')}%", 'status': "atingido" if avg_sla >= sla_goal else "nao-atingido", 'trend': sla_trend},
            {'label': "Hosts com SLA < 99.9%", 'value': df_sla[df_sla['SLA (%)'] < 99.9].shape[0], 'sublabel': f"De um total de {len(hosts_for_sla)} hosts", 'status': "critico" if df_sla[df_sla['SLA (%)'] < 99.9].shape[0] > 0 else "ok", 'trend': None},
            {'label': "Total de Incidentes", 'value': len(all_problems), 'sublabel': "Eventos de problema registrados", 'status': "info", 'trend': incidents_trend},
            {'label': "Principal Ofensor", 'value': principal_ofensor, 'sublabel': "Host com mais incidentes", 'status': "info", 'trend': None}
        ]
    ...
    def get_items(self, hostids, filter_key, search_by_key=False, exact_key_search=False):
        ...
        params = {'output': ['itemid', 'hostid', 'name', 'key_'], 'hostids': hostids, 'sortfield': 'name'}
        if search_by_key:
    ...
    def obter_eventos(self, object_ids, periodo, id_type='hostids', max_depth=3):
        ...
        if max_depth <= 0:
            ...
            return None
        params = {'output': 'extend', 'selectHosts': ['hostid'], 'time_from': time_from, 'time_till': time_till, id_type: object_ids, 'sortfield': ["eventid"], 'sortorder': "ASC", 'select_acknowledges': 'extend'}
    ...
    def obter_eventos_wrapper(self, object_ids, periodo, id_type='objectids'):
        ...
        if all_events is None:
            current_app.logger.critical(f"Falha crítica ao coletar eventos para os IDs. Abortando.")
            return None
    ...
    def _count_problems_by_host(self, problems, all_hosts):
            ...
            if p.get('object') == '0' and p.get('hosts'):
                ...
                if host_name:
                    problem_data.append({
                        ...
                        'Ocorrências': 1, # Cada problema é uma ocorrência
                        'clock': p['clock']
                    })
        ...
        # Agrupa os dados para contar as ocorrências
